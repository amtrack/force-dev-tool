...
module.exports.fromPackageXml = function(packageXml) {
	var parsed = new xmldoc.XmlDocument(packageXml);
	var manifestJSON = [];
	parsed.childrenNamed('types').forEach(function(types) {
		var type = types.childrenNamed('name')[0].val;
		types.childrenNamed('members').forEach(function(member) {
			var component = new MetadataComponent(type + '/' + member.val);
			manifestJSON.push(component);
		});
	});
	var apiVersion = parsed.childrenNamed('version')[0].val;
	return new Manifest({
...
Manifest.prototype.filterStandard = function() {
	...
	return _.filter(self.manifestJSON, function(component) {
		if (['AppMenu', 'AuraDefinitionBundle', 'Community', 'CustomApplication', 'Layout', 'ListView', 'Profile', 'RecordType', 'Role', 'WebLink', 'Workflow'].indexOf(component.type) >= 0) {
...
Manifest.prototype.toPackageXml = function(destructive) {
	...
	metadataXML += "<Package xmlns=\"http://soap.sforce.com/2006/04/metadata\">\n";
	j.types.forEach(function(t) {
		metadataXML += "    <types>\n";
		t.members.forEach(function(member) {
			metadataXML += "        <members>" + member + "</members>\n";
		});
		metadataXML += "        <name>" + t.name[0] + "</name>\n";
		metadataXML += "    </types>\n";
	});
	if (!destructive) {
		metadataXML += "    <version>" + j.version[0] + "</version>\n";
	}
	metadataXML += "</Package>\n";
	return metadataXML;
};
