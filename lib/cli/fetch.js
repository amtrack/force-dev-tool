...
var doc = "Usage:\n" +
	...
	"	--no-recursive       Don't fetch children.\n" +
...
SubCommand.prototype.complete = function(tabtab, data) {
	return tabtab.log(['-a', '--all', '--use-forceignore', '--progress', '--api-versions-only', 'REMOTENAME'], data, '');
};
...
SubCommand.prototype.process = function(proc, callback) {
	...
	if (!self.opts['--no-recursive']) {
		describeRemoteOpts.recursive = true;
	}
