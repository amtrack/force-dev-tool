...
SubCommand.prototype.process = function(proc, callback) {
	...
	var client;
	try {
		client = self.project.remotes.get(self.opts['<remote>']);
	}
	catch (err) {
		return callback(err);
	}
	...
	console.log('Retrieving from remote ' + chalk.cyan(client.name) + ' to directory ' + chalk.cyan(path.normalize(path.relative(proc.cwd, targetDirectory))));
	client.login(function(loginErr) {
		if (loginErr) {
			return callback(loginErr);
		}
		client.conn.metadata.pollTimeout = config.pollTimeout;
		var r = client.conn.metadata.retrieve({unpackaged: currentPackageXml.getJSON(), apiVersion: currentPackageXml.apiVersion});
		if (showProgress) {
			var polls = 1;
			...
			fs.writeFile(zipFileName, result.zipFile, {encoding: 'base64'}, function(writeErr) {
				...
				yauzl.open(zipFileName, function(openErr, zipfile) {
					...
					zipfile.once("close", function() {
						fs.move(path.resolve('unpackaged'), targetDirectory, {clobber: true}, function(mvErr) {
							if (mvErr) {
								return callback(mvErr);
							}
							if (keepZip) {
								// finished
