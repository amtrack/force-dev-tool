...
SubCommand.prototype.process = function(proc, callback) {
	...
	var client;
	try {
		client = self.project.determineRemote(self.opts['<remote>']);
	}
	catch (err) {
		return callback(err);
	}
	...
	console.log('Retrieving from remote ' + chalk.cyan(client.name) + ' to directory ' + chalk.cyan(path.relative(proc.cwd, retrieveRoot)));
	var localPackageXml = Manifest.fromPackageXml(CliUtils.readPackageXml(path.join(retrieveRoot, 'package.xml')));
