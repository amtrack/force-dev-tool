...
var doc = "Usage:\n" +
	...
	"	--objects              Remove various customizations from standard objects.\n" +
...
SubCommand.prototype.process = function(proc, callback) {
	...
	var customRegex = new RegExp('^.*(__c|__mdt|__kav)$');
	if (self.opts['--objects']) {
		var objects = fetchResult.metadata['CustomObject'];
		if (!objects) {
			return;
		}
		objects.forEach(function(object) {
			var fullName = object.fullName;
			if (customRegex.test(fullName)) {
				return;
			}
			var newObject = {
				fullName: fullName
			};
			// empty all searchLayouts
			newObject.searchLayouts = {};
			if (object.actionOverrides) {
				newObject.actionOverrides = object.actionOverrides;
				newObject.actionOverrides.forEach
				if (!Array.isArray(newObject.actionOverrides)) {
					newObject.actionOverrides = [newObject.actionOverrides];
				}
				for (var i = 0; i < newObject.actionOverrides.length; i++) {
					if (newObject.actionOverrides[i].type !== "Default") {
						newObject.actionOverrides[i] = {
							actionName: newObject.actionOverrides[i].actionName,
							type: "Default"
						}
					}
				}
			}
			var metadataXml = new MetadataWriter('CustomObject', newObject).toString();
			var v = new MetadataFile({
				path: path.join('objects', fullName + '.object'),
	...
	if (self.opts['--layouts']) {
		...
		layouts.forEach(function(layout) {
			...
			if (layout.customConsoleComponents) {
				delete layout.customConsoleComponents;
			}
			if (layout.feedLayout) {
				delete layout.feedLayout;
			}
			if (layout.summaryLayout) {
				delete layout.summaryLayout;
			}
			// if (layout.quickActionList) {
			// 	// TODO: only remove custom quickactions
			// 	delete layout.quickActionList;
			// }
			if (layout.platformActionList) {
				delete layout.platformActionList;
			}
			if (layout.miniLayout) {
				delete layout.miniLayout;
			}
							...
							if (layoutColumn.layoutItems) {
								...
								for (var i = 0; i < layoutColumn.layoutItems.length; i++) {
									if (layoutColumn.layoutItems[i].customLink) {
										...
										delete layoutColumn.layoutItems[i];
									} else if (layoutColumn.layoutItems[i].field && customRegex.test(layoutColumn.layoutItems[i].field)) {
