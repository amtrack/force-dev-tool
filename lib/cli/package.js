...
var doc = "Usage:\n" +
	...
	"	--remote-api-version   Use latest API version from remote\n" +
...
SubCommand.prototype.complete = function(tabtab, data) {
	return tabtab.log(['-a', '--all', '--remote-api-version', '--custom-only', '--template', '--target', 'fmt', 'REMOTENAME'], data);
};
...
SubCommand.prototype.process = function(proc, callback) {
	...
	if (self.opts['--remote-api-version']) {
		manifest.apiVersion = fetchResult.getApiVersion() || config.get('defaultApiVersion');
	} else {
