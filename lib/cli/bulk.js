...
SubCommand.prototype.complete = function(tabtab, data) {
	...
	return tabtab.log(['export', 'insert', 'update', 'upsert', 'delete', 'hardDelete', '--in', '--out', '--extIdField', '--pollInterval', '--pollTimeout'], data, '');
};
...
SubCommand.prototype.process = function(proc, callback) {
	...
	if (self.opts.export) {
		...
		return callback(new Error('Unsupported action'));
	}
	...
	remote.login(function(loginErr) {
		if (loginErr) {
			return callback('Login failed for remote ' + remote.name + ': ' + loginErr);
		}
		remote.conn.bulk.pollInterval = pollInterval;
		remote.conn.bulk.pollTimeout = pollTimeout;
		if (self.action === 'query') {
			if (self.opts['<SOQL>'] !== '-') {
				inputStream = new stream.Readable({ encoding: 'utf8' });
			}
			var soql = '';
			...
			remote.conn.bulk.load(self.sObjectName, self.action, loadOptions, inputStream, function(loadErr, response) {
				if (loadErr) {
