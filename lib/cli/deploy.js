...
SubCommand.prototype.process = function(proc, callback) {
	...
	if (self.action === 'Test execution') {
		...
		console.log('Running ' + chalk.cyan(self.action) + ' of directory ' + chalk.cyan(path.relative(proc.cwd, deployRoot)) + ' to remote ' + chalk.cyan(client.name));
	}
	...
	client.login(function(loginErr) {
		...
		client.conn.metadata.deploy(self.archive, self.deployOpts).complete({
			details: true
		}, function(err, res) {
			if (err) {
				return callback(err);
