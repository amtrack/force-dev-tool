...
var SubCommand = module.exports = function(doc, project) {
	var self = this;
	Command.call(self, doc ? doc : deployDoc, project);
...
SubCommand.prototype.process = function(callback) {
	...
	archive.directory(deployRoot, path.basename(deployRoot));
	archive.finalize();
	...
	client.login(function(loginErr) {
		...
		client.conn.metadata.deploy(archive, self.deployOpts).complete({details: true}, function(err, res) {
			if (err) {
				callback(err);
			}
