...
MetadataContainer.prototype.filter = function(manifest) {
	...
	if (manifest) {
		...
		componentFileNames.forEach(function(componentFileName) {
			...
			var type = self.describeMetadataService.getType(component.type);
			if (!type) {
			...
			if (type.childXmlNames) {
				...
				if (containerComponent && containerComponent.length) {
					...
					// TODO: construct a new file with only the related components
					var allComponents = vinylContainer.getComponents();
					var components = [];
					relatedComponents.forEach(function(cmp) {
						var cmpNameParts = cmp.fullName.split('.');
						var cmpName = cmpNameParts.length > 1 ? cmpNameParts[1] : cmpNameParts[0];
						var c = allComponents[cmp.type][cmpName];
						if (c) {
							components.push(c);
						}
					});
					var content = '<?xml version="1.0" encoding="UTF-8"?>\n' + '<' + type.xmlName + ' xmlns="http://soap.sforce.com/2006/04/metadata">\n' + components.join("\n") + '\n</' + type.xmlName + '>\n';
					var containerFile = new MetadataFile({
						path: componentFileName,
...
MetadataContainer.prototype.index = function() {
	...
	self.vinyls.forEach(function(vinyl) {
		...
		if (type.childXmlNames) {
			var childComponents = vinyl.getComponents();
			if (childComponents) {
				...
				self.manifest.merge(childManifest);
			}
		} else {
...
MetadataContainer.prototype.diff = function(other) {
	...
	allFileNames.forEach(function(filename) {
		...
		if (from && to) {
			...
			metadataContainer.manifest.merge(diffResult.manifest);
			metadataContainer.destructiveManifest.merge(diffResult.destructiveManifest);
		} else if (from) {
