...
MetadataFileContainer.prototype.diff = function(other) {
	...
	if (isDeleted && componentFrom) {
		...
		if (parsed) {
			...
			_.keys(m1).sort(MetadataUtils.compareMetadataTypeNames).forEach(function(type) {
				m1[type] = _.keyBy(m1[type], 'fullName');
			});
			var m2 = _.groupBy(other.components, 'type');
			_.keys(m2).sort(MetadataUtils.compareMetadataTypeNames).forEach(function(type) {
				m2[type] = _.keyBy(m2[type], 'fullName');
			});
...
MetadataFileContainer.prototype.getGroupedAndSortedComponents = function() {
	...
	_.keys(map).sort(MetadataUtils.compareMetadataTypeNames).forEach(function(metadataType) {
...
MetadataFileContainer.diffMaps = function(mapA, mapB) {
	...
	_.keys(mapB).sort(MetadataUtils.compareMetadataTypeNames).forEach(function(type) {
		_.keys(mapB[type]).sort(MetadataUtils.compareMetadataTypeNames).forEach(function(member) {
	...
	_.keys(mapA).sort(MetadataUtils.compareMetadataTypeNames).forEach(function(type) {
		_.keys(mapA[type]).sort(MetadataUtils.compareMetadataTypeNames).forEach(function(member) {
...
// STATIC
var builtInProps = ['diff', 'getManifest', 'parse', 'addComponent', 'getGroupedAndSortedComponents', 'contentsWithoutChilds', 'writeContents', 'updateContents', 'toString', 'diffMaps'];
