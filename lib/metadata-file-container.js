...
MetadataFileContainer.prototype.contentsWithoutChilds = function(includedChildComponents = []) {
	...
	if (childComponents) {
		...
		var tagIncludedChildNames = includedChildComponents.map(child => describeMetadataService.getType(child).tagName);
		var tagChildNames = childComponents.map(child => describeMetadataService.getType(child).tagName)
				.filter(child => !tagIncludedChildNames.includes(child));
...
MetadataFileContainer.prototype.updateContents = function(added, modified) {
	var self = this;
	var includedChildComponents = modified.manifestJSON.map(m => m.type)
	    .concat(added.manifestJSON.map(m => m.type)).filter(m => m !== undefined);
