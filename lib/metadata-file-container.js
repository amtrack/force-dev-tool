...
MetadataFileContainer.prototype.parse = function() {
	...
	//set CustomObject contents to required fields so we can include when adding non-named metadata
	if (type.xmlName === 'CustomObject') {
		var fields = describeMetadataService.getCustomObjectRequiredFields();
		var contents = _.reduce(fields, function(result, field) {
			var child = parsed.childNamed(field);
			if (child) {
		...
		if (contents.length > 0) {
			self.required = new Buffer(contents.join('\n'));
		}
	}
	return self;
};
...
MetadataFileContainer.prototype.addComponent = function(cmp) {
	var self = this;
	var metadataType = cmp.getMetadataType();
	if (metadataType && !metadataType.isNamed) {
		self.hasNonNamed = true;
	}
...
MetadataFileContainer.prototype.toString = function() {
	...
	if (self.hasNonNamed) {
		lines.push(self.opts.contents);
	}

	lines.push("</" + type.xmlName + ">");
	lines.push("");
	return beautify_html(lines.join("\n"),{end_with_newline: true});
};
