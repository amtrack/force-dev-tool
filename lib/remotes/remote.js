...
Remote.prototype.fetch = function(callback) {
	...
	self.login(function(loginErr) {
		if (loginErr) {
			callback('Login failed for remote ' + remoteName + ': ' + loginErr);
		}
		...
		async.series({
			...
			manifest: function(cb) {
				...
				var childXmlNames = _.pluck(_.filter(self.types.metadataObjects, function(type){ return type.childXmlNames; }), 'childXmlNames');
				metadataTypeNames = _.flatten([].concat(metadataTypeNames, childXmlNames));
				var queries = metadataTypeNames.map(function(item) { return {type: item}; });
				...
				var inFolderQueries = folderBasedMetadataTypes.map(function(item) { return {type: item }; });
				self.listMetadata(inFolderQueries, function(inFolderErr, foldersManifestJSON){
					if (inFolderErr) {
						cb(inFolderErr);
					}
					if (foldersManifestJSON) {
						var folderBasedQueries = foldersManifestJSON.map(function(item) { return {type: folderBasedMetadataMap[item.type], folder: item.fullName}; });
			...
			toolingObjects: function(cb) {
				self.query("SELECT Id,Name,TableEnumOrId FROM ApexTrigger WHERE Status = 'Active'", cb);
			},
