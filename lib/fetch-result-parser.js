...
FetchResultParser.prototype.filterStandard = function() {
	...
	var self = this;
	var customParentRegex = new RegExp('^.*(__c|__mdt|__kav)\\..*$');
	var customChildRegex = new RegExp('^.*(__c|__mdt|__kav)$');
	self.fileProperties = _.filter(self.fileProperties, function(fileProperty) {
		if (["Layout", "FlowDefinition"].indexOf(fileProperty.type) >= 0) {
			// generate empty or deactivate before
			return false;
		}
		if (["Workflow", "Settings", "Translations", "ListView", "LiveChatButton", "LiveChatDeployment", "Skill", "MatchingRule", "MatchingRules", "EscalationRules", "CustomObjectTranslation", "Community", "AppMenu", "AssignmentRules", "AutoResponseRules", "CompactLayout", "Certificate", "RecordType", "StandardValueSet", "StandardValueSetTranslation", "Portal", "CleanDataService", "BusinessProcess"].indexOf(fileProperty.type) >= 0) {
			return false;
		}
		...
		if (["CustomObject"].indexOf(fileProperty.type) >= 0 && !(customChildRegex.test(fileProperty.fullName))) {
			return false;
		}
		if (["CustomField"].indexOf(fileProperty.type) >= 0 && !(customChildRegex.test(fileProperty.fullName))) {
			return false;
		}
		if (["CustomField", "BusinessProcess", "CompactLayout", "RecordType", "WebLink", "ValidationRule", "SharingReason", "ListView", "FieldSet"].indexOf(fileProperty.type) >= 0 && customParentRegex.test(fileProperty.fullName)) {
			// don't list child metadata of custom objects
			return false;
		}
		...
		if (["EmailTemplate", "Dashboard", "Document", "Report"].indexOf(fileProperty.type) >= 0) {
			if (!(new RegExp('.*\/.*').test(fileProperty.fullName))) {
				...
				return false;
			} else if (new RegExp('unfiled\\$public.*').test(fileProperty.fullName) || new RegExp('.*CustomerPortal.*').test(fileProperty.fullName) || new RegExp('.*SupportSelfService.*', 'i').test(fileProperty.fullName) || new RegExp('.*SupportCase.*', 'i').test(fileProperty.fullName)) {
