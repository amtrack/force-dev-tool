...
FetchResultParser.prototype.transform = function() {
	...
	self.fileProperties = self.fileProperties.map(function(fileProperty) {
		...
		} else if (fileProperty.type === 'Layout' && fileProperty.manageableState === 'installed' && fileProperty.namespacePrefix) {
			// https://gearset.com/blog/retrieving-and-deploying-namespaced-layouts-in-salesforce
			// the dash character is the separator between object name and layout name, but the layout name may contain dashes itself
			var layoutFullNameParts = fileProperty.fullName.split('-');
			var nsObject = layoutFullNameParts[0];
			var layoutName = layoutFullNameParts.slice(1, layoutFullNameParts.length).join('-');
			if (layoutName.indexOf(fileProperty.namespacePrefix + '__') !== 0) {
				var fixedLayoutName = nsObject + '-' + fileProperty.namespacePrefix + '__' + layoutName;
				fileProperty.fullName = fixedLayoutName;
				fileProperty.fileName = path.join('layouts', fixedLayoutName) + '.layout';
			}
		} else if (fileProperty.type === 'QuickAction' && fileProperty.manageableState === 'installed' && fileProperty.namespacePrefix) {
			var fixedQuickActionName = fileProperty.fullName.split('.').map(function(part) {
				if (part.indexOf(fileProperty.namespacePrefix + '__') !== 0) {
					return fileProperty.namespacePrefix + '__' + part;
				}
				return part;
			}).join('.');
			if (fixedQuickActionName !== fileProperty.fullName) {
				fileProperty.fileName = path.join('quickActions', fixedQuickActionName) + '.layout';
				fileProperty.fullName = fixedQuickActionName;
			}
