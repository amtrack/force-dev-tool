...
FetchResultParser.prototype.transform = function() {
	...
	self.fileProperties = self.fileProperties.map(function(fileProperty) {
		...
		} else if (['CustomMetadata', 'QuickAction'].indexOf(fileProperty.type) >= 0 && fileProperty.manageableState === 'installed' && fileProperty.namespacePrefix) {
			var fixedCmpFullName = fileProperty.fullName.split('.').map(function(part) {
